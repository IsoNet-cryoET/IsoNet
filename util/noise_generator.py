...
class NoiseMap:
    ...
    @staticmethod
    def refresh(size_big, filter = 'ramp' , ncpus = 1, missingAngle=(30,30)):
        NoiseMap.noise_map = simulate_noise([size_big, filter, ncpus, missingAngle])
...
def simulate_noise(params):
    global angles
    angles = np.arange(params[3][0]-90,90-params[3][1],3)
    size = params[0]
...
def make_noise_folder(noise_folder,noise_filter,cube_size,num_noise=1000,ncpus=1,large_side=500, missingAngle = (30,30)):
    mkfolder(noise_folder)
    print('generating large noise volume; mode: {}'.format(noise_filter))
    NoiseMap.refresh(large_side, noise_filter, ncpus, missingAngle)
