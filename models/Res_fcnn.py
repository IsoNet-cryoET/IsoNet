    ...
    def _func(last_layer):
        ...
        if len(kernel)==2:
            ...
            conv=Conv3D(n_filter,kernel,
                        padding=padding,
                        kernel_regularizer=regularizers.l2(0.00007),
                        kernel_initializer=init,**kwargs)(last_layer)
        if batch_norm:
...
def res_unet_block(filter_base=32,depth=2,convs_per_depth=2,
               kernel=(3,3),
               batch_norm=False,
               dropout=0.0,
               pool=(2,2)):
    ...
    def _func(inputs):
        concatenate = []
        layer = inputs
        layer = Conv3D(filter_base, (1,1,1))(layer)
        for n in range(depth):
            jump = layer
            for i in range(convs_per_depth):
                if i%2 == 0 and i>0 :
                    layer = Add()([layer,jump])
                    jump = layer
            ...
            layer = Conv3D(filter_base*2**(n+1), (1,1,1))(layer)
        jump = layer     
        for i in range(convs_per_depth):
            if i%2 == 0 and i>0 :
                layer = Add()([layer,jump])
        ...
        layer = conv_blocks(filter_base*2**max(0,depth-1),(1,1,1),dropout=dropout,
                            batch_norm=batch_norm,name="middle_%s" % convs_per_depth)(layer)

        for n in reversed(range(depth)):
            print('depth is : %d' % n)

            layer = Concatenate(axis=-1)([layer,concatenate[n]])
            layer = Conv3D(filter_base * 2 ** n, (1,1,1))(layer)
            jump = layer
            for i in range(convs_per_depth):
                if i%2 == 0 and i>0 :
                    layer = Add()([jump,layer])
                    jump = layer
                layer = conv_blocks(filter_base * 2 ** n, kernel, dropout=dropout,
                                    batch_norm=batch_norm,name="up_level_%s_no_%s" % (n, i))(layer)
            layer = conv_blocks(filter_base * 2 ** max(0,(n-1)), (1,1,1), dropout=dropout,
...
def train(X,Y, val_data, outFile, epochs=20,steps_per_epoch=64,batch_size=128,n_gpus=6):
    ...
    Unet = res_unet_block(filter_base=filter_base, depth=depth,convs_per_depth=convs_per_depth,
                      kernel=kernel,
...
def train3D_seq(outFile, data_folder = 'data', epochs=40, steps_per_epoch=128,batch_size=64,dropout = 0.3,filter_base=32, convs_per_depth = 3,kernel = (3,3,3), pool = (2,2,2), batch_norm = False, depth = 3, n_gpus=2):
